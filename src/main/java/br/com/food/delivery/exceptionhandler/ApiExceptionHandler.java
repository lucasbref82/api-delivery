...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleUncaught(Exception ex, WebRequest request) {
		...
		Problema problem = createProblemBuilder(status, tipoProblema, detail).mensagem(detail).build();
	...
	protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
		...
		Problema problem = createProblemBuilder(status, tipoProblema, detail).mensagem(MSG_ERRO_GENERICA_USUARIO_FINAL)
				.build();
	...
	public ResponseEntity<Object> handlePropertyValueException(PropertyValueException ex, WebRequest request){
		HttpStatus status = HttpStatus.CONFLICT;
		TipoProblema tipoProblema = TipoProblema.VIOLACAO_CONSTRAINT;
		String detail = String.format("O campo %s da entidade %s não ser nulo.", ex.getPropertyName(), ex.getEntityName());
		Problema problema = createProblemBuilder(status, tipoProblema, detail)
				.mensagem(String.format("O campo %s não pode ser vazio.", ex.getPropertyName())).build();
	...
	private Problema.ProblemaBuilder createProblemBuilder(HttpStatus status, TipoProblema tipoProblema, String detail) {
		...
		return Problema.builder().dataHora(LocalDateTime.now()).status(status.value()).tipo(tipoProblema.getUri())
				.titulo(tipoProblema.getTitle()).detalhes(detail);
	}
